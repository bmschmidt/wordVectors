...
We begin by importing the `wordVectors` package and the `maggritr` package, because its pipe operator makes things much clearer.
...
3. `iter` is how many times to read through the corpus. With fewer than 100 books, it can greatly help to increase the number of passes; if you're working with billions of words, it probably matters less.
4. Training can take a while. On my laptop, it takes a few minutes to train these cookbooks; larger models take proportionally more time.
