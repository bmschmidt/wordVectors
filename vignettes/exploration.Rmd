...
demo_vectors %>% extract_vectors(c("lady","woman","man","he","she","guy","man")) %>% 
  plot(method="pca")
...
First we use `closest_to` to build up a data.frame of every similarity `"good" -"bad"` and `"woman" - "man"`. 
...
evaluative_words = demo_vectors %>% 
  closest_to(c(~ "good"-"bad", ~"she" - "he"), n=Inf) 

filter_list = demo_vectors %>% closest_to(~"good" + "bad", n=75) %>% select(word)
