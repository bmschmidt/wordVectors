...
cat(demo,file = "/tmp/input.txt")

if (file.exists("/tmp/tmp.txt")) file.remove("/tmp/tmp.txt")


test_that("Preparation produces file",
          expect_equal(
            prep_word2vec("/tmp/input.txt","/tmp/tmp.txt"),
            "/tmp/tmp.txt"
          )
)
...
test_that("Bundling works on multiple levels",
          expect_equal(
            prep_word2vec("/tmp/input.txt","/tmp/tmp.txt",bundle_ngrams = 3),
            "/tmp/tmp.txt"
          )
)
